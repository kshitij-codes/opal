...
export const useSearch = ({ key, type }: { key: string; type: "USERS" }) => {
  ...
  const onSearchQuery = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(delayInputTimerId);
  }, [query]);
